...
for branch in ${BRANCH_NAME//,/ }; do
  ...
  if [ -n "$branch" ] && [ -n "$devices" ]; then
    ...
    if [ "$CALL_REPO_INIT" = true ]; then
      ...
      if [ "$LOCAL_MIRROR" = true ]; then
        ( yes||: ) | repo init -u https://github.com/Linux-on-LineageOS/android.git --reference "$MIRROR_DIR" -b "$branch" --git-lfs &>> "$repo_log"
      else
        ( yes||: ) | repo init -u https://github.com/Linux-on-LineageOS/android.git -b "$branch" --git-lfs &>> "$repo_log"
      fi
    else
