...
if [ "$LOCAL_MIRROR" = true ]; then
  ...
  if [ "$INIT_MIRROR" = true ]; then
    if [ ! -d .repo ]; then
      ...
      ( yes||: ) | repo init -u https://github.com/Linux-on-LineageOS/mirror --mirror --no-clone-bundle -p linux --git-lfs &>> "$repo_log"
    fi
  else
...
for codename in ${devices//,/ }; do
  if [ -n "$codename" ]; then
    ...
    if [ "$CALL_REPO_INIT" = true ]; then
      ...
      if [ "$LOCAL_MIRROR" = true ]; then
        ( yes||: ) | repo init -u https://github.com/Linux-on-LineageOS/android.git --reference "$MIRROR_DIR" -b "$branch" -g default,-darwin,-muppets,muppets_"${codename}" --git-lfs &>> "$repo_log"
      else
        ( yes||: ) | repo init -u https://github.com/Linux-on-LineageOS/android.git -b "$branch" -g default,-darwin,-muppets,muppets_"${codename}" --git-lfs &>> "$repo_log"
      fi
    else
